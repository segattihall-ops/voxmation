...
import { Phone, PhoneIncoming, Mic, CheckCircle2 } from "lucide-react";
  ...
  return (
    <div className="flex items-center gap-[3px] h-8">
      {Array.from({ length: WAVE_BARS }).map((_, i) => (
        <motion.div
          key={i}
          className="w-1 rounded-full bg-[#FF8A1F]"
          animate={active ? {
...
export default function DemoCallWidget() {
  ...
  const reset = () => { setStep(-1); setRunning(false); setDone(false); };
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <h2 className="font-display text-4xl sm:text-5xl font-extrabold text-[#F7F5F0] leading-tight mb-6">
              Watch a Call Get<br />
              <span className="text-orange">Handled in Real Time</span>
            </h2>
            <p className="text-lg text-[#8A99B3] font-body mb-8 leading-relaxed">
              The moment a call comes in, VOXmatiON answers, qualifies the lead, and routes it — without you lifting a finger.
              See it happen live.
            </p>
            ...
            <div className="w-full max-w-sm glass rounded-3xl border-subtle p-6 relative overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                  <div className={`w-3 h-3 rounded-full transition-colors duration-500 ${running ? "bg-green-400" : "bg-[rgba(255,255,255,0.2)]"}`} />
                ...
                {CALL_STEPS.map((s, i) => {
                  ...
                  return (
                    <motion.div
                      key={s.id}
                      animate={{
                        opacity: complete ? 1 : active ? 1 : 0.3,
                        scale: active ? 1.02 : 1,
                      }}
                    ...
                    >
                      ...
                      {active && (
                        <motion.div
                          className="w-2 h-2 rounded-full bg-[#FF8A1F]"
              ...
              <AnimatePresence mode="wait">
                {!running && !done && (
                  <motion.button
                    ...
                    exit={{ opacity: 0, y: -8 }}
                    onClick={startDemo}
                    className="w-full py-3 rounded-xl bg-[#FF8A1F] text-white font-bold text-sm font-body hover:bg-[#FFB347] transition-colors flex items-center justify-center gap-2"
                  >
                ...
                {running && (
                  ...
                  >
                    <p className="text-xs text-[#8A99B3] font-body">AI handling call...</p>
                  </motion.div>
                )}
                {done && (
                  <motion.div
                  ...
                  >
                    <div className="text-center py-2">
                      <p className="text-xs font-semibold text-green-400 font-body">Call complete — job booked!</p>
                    </div>
                    <button
                      onClick={reset}
                      className="w-full py-2.5 rounded-xl border border-[rgba(255,138,31,0.3)] text-[#FF8A1F] text-xs font-semibold font-body hover:bg-[rgba(255,138,31,0.08)] transition-colors"
                    >
                      Run Again
                    </button>
                  </motion.div>
